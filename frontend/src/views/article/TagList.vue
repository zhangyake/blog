...
export default {
  ...
  methods: {
    loadTags () {
      this.$api.getAllTags().then(response => {
    ...
    createOrUpdateTag () {
      ...
      if (this.tagToCreate.id) {
        this.$api.updateTag({ id: this.tagToCreate.id, name: this.tagToCreate.name }).then(response => {
          this.$message.success('更新成功！')
          this.loadTags()
          this.tagToCreate = {}
        })
      } else {
        this.$api.insertTag(this.tagToCreate).then(response => {
